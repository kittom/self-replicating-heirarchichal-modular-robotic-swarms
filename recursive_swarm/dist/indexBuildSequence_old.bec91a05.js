...
function decisionTree() {
    ...
    setup.nodes.push({
        ...
        name: "Decision Tree\nR\u2074:" + Robots[4].num + ", R\xb2:" + Robots[2].num + ", R:" + Robots[1].num + "",
        size: scale,
        min: false,
        parent: "top",
        c: color66,
    ...
    Graph(document.getElementById("graph")).graphData({
        nodes,
        links
    });
}
function decision(setup, robots, size, name) {
    ...
    if (robots[size].num < robots[size].maxNum) {
        ...
        setup.nodes.push({
            ...
            name: "Reproduce\nR\u2074:" + r[4].num + ", R\xb2:" + r[2].num + ", R:" + r[1].num + "",
            size: scale,
            parent: name,
            min: false,
            c: color88,
...
function estimateCost(robots, path) {
    ...
    for(var i = 0; i < buildList.listZ.length; i++){
        binZ = buildList.listZ[i];
        indexZ = i;
        maxIndexSize = binZ.maxSizeIndex;
        for(var j = 0; j < binZ.listSize.length; j++){
            binSize = binZ.listSize[j];
            indexSize = j;
            maxIndexSDF = binSize.maxSDFIndex;
            for(var k = 0; k < binSize.listSDF.length; k++){
                binSDF = binSize.listSDF[k];
                indexSDF = k;
                var cubesToBuild = [];
                var size = binSize.size;
                if (robots[size].num > 0) //there is robots same size
                cubesToBuild = binSDF.listCubes;
                    ...
                    for(var l = 0; l < binSDF.listCubes.length; l++)for(var ii = 0; ii < voxelIndexList[2].list.length; ii++){
                        ...
                        if (buildList.listZ[i].listSize[j].size == 1) stockBuilt = true;
                        cubesToBuild.push({
                    ...
                    for(var l = 0; l < binSDF.listCubes.length; l++)for(var ii = 0; ii < voxelIndexList[4].list.length; ii++){
                ...
                if (cubesToBuild.length / robots[size].num - Math.floor(cubesToBuild.length / robots[size].num) > 0) numRobotParallel++;
                //get furthest robot
                var furthestVoxel = new THREE.Vector3(0, 0, 0);
                for(var l = 0; l < cubesToBuild.length; l++)if (cubesToBuild[l].position.x + cubesToBuild[l].position.y + cubesToBuild[l].position.z > furthestVoxel.x + furthestVoxel.y + furthestVoxel.z) furthestVoxel = cubesToBuild[l].position.clone();
            ...
            for(var k = 0; k < json[i][j].length; k++){
                ...
                for(var l = 0; l < json[i][j][k].length; l++){
                    ...
                    buildList.listZ[i].listSize[j].listSDF[k].listCubes.push({
                        ...
                        stockAssigned: false,
                        stockPickedUp: false,
                        position: new THREE.Vector3(json[i][j][k][l].origin[0] * scale + shift, json[i][j][k][l].origin[2] * scale + shift, json[i][j][k][l].origin[1] * scale + 1)
                    });
                    setup.nodes.push({
                        path: "[" + i + "," + j + "," + k + "," + l + "]",
            ...
            for(var k = 0; k < json[i][j].length; k++){
                ...
                for(var l = 0; l < json[i][j][k].length; l++){
                    ...
                    if (json[i][j][k][l].origin[2] * scale + nomSize / 2.0 < 0 || json[i][j][k][l].origin[0] * scale + nomSize / 2.0 < 0) {
                        ...
                        if (nomSize == 2) for(var ii = 0; ii < voxelIndexList[2].list.length; ii++){
                            var nomSize1 = 1;
                        ...
                        if (nomSize == 4) for(var ii = 0; ii < voxelIndexList[4].list.length; ii++){
                            var nomSize1 = 1;
...
//# sourceMappingURL=indexBuildSequence_old.bec91a05.js.map
